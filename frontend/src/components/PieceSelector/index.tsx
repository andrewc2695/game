...
interface IPieceSelector {
	...
	left: number;
	id: string;
	setShowSelector: React.Dispatch<
		...
		<div className="BackGround" onClick={() => setShowSelector(undefined)}>
			<div className="PieceSelector" style={{ top, left }}>
				{Object.entries(board.getAvaliablePieces()).map(([k, v]) => {
					...
					const isStartingPieceValid =
						board.canPieceStartHere(k as "flag", pos as [number, number]) &&
						v > 0;
					return (
						<div
							className={`PieceSelector-Piece ${
								isStartingPieceValid ? "" : "PieceSelector-Invalid"
							}`}
							key={k}
						>
							<div
